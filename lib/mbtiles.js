...
util.inherits(MBTiles, require('events').EventEmitter)
...
var DataGatherer = function() {
    stream.Transform.call(this, {
        objectMode: true
    });

    var Assembler = function(pipes) {
        stream.Transform.call(this);
        ...
        this._flush = function(callback) {
            return async.forEach(pipes, function(dst, done) {
                return dst.write({
                    z: z,
                    x: x,
                    y: y,
    ...
    this._transform = function(obj, encoding, callback) {
        // if nothing is connected, don't bother reading the stream we just got
        if (this._readableState.pipesCount === 0) {
            return callback();
        }

        // nothing is written directly downstream; Assembler takes the
        // destinations and feeds them
        return obj
            .pipe(new Assembler(this._readableState.pipes))
            .on("finish", callback);
    };
};

util.inherits(DataGatherer, stream.Transform);
...
var Writable = function(sink) {
    ...
    // TODO call sink.stopWriting once the stream is finished
    // on("finish") doesn't seem to work properly
    ...
    var writable = false,
        putTile = function(obj, callback) {
            console.log(obj.z, obj.x, obj.y, obj.data.length);
            return sink.putTile(obj.z, obj.x, obj.y, obj.data, callback);
        };

    this._write = function(obj, encoding, callback) {
        // this assumes that the source stream has been already gathered into
        // obj.data using DataGatherer or something similar
        if (!writable) {
...
MBTiles.prototype.createWriteStream = function() {
    // we want to return a reference to the head-end of the pipeline
    var writeStream = new DataGatherer();
